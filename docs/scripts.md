...
We deliberately limit the amount of data in these exports by providing only one tooth or half-tooth. The reason: Fusion tends to slow down significantly when dealing with large polylines or large meshes. It is best to work in Fusion as long as possible on a single sector, and only create the entire tooth (using a circular pattern and combine-bodies) at the last moment. For example for 3D printing. It is also important not to exaggerate the requested tolerance, as tighter tolerance increases the size of the exports.

By default, the exports include a point outside of the tooth, which is a linear extrapolation of the outer element of the tooth. This allows, for example, to create a body that is used in Fusion as a knife, cutting out a tooth from a cylindrical sector instead of being used additive. In any case, the outer point can be deleted if not needed, or the mesh can be clipped at the gear outer radius. 
...
Our CSV exports satisfies these requirements:
- a header is added.
- Z values are provided, where Z = 0 is set for our 2D data.
- output is converted to cm units.
...
The file then contains, in units of mm:
- vertices
- faces
...
When CNC milling gears in this way, the fillet radius is an important factor in the process. Fillet radius must be larger than tool radius plus any stock to leave. And even without stock to leave, it is not good practice to give the mill some room for manoeuvre at the bottom of the fillet.
...
When designing a gear train, it is typical to know the desired end-to-end ratio, but not how this should be broken down over the various stages. For example, one may break down a 120 ratio as 4, 5, 6, or 4, 5, 2, 3. These choices depend on design constraints, e.g. one wants to give the first stage pinion the maximum number of teeth, or to fit the train inside some space.

We provide two scripts that can help design on this point. The user provides:
- the target end-to-end ratio
- an array of permitted pinion tooth counts, for example 7, 8, 9. 
...
- the parameters to create a pinion - the usual set
